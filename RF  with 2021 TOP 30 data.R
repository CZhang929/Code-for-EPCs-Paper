...
#With 2021 data
##################################################################
data <- read.csv("cleaned_2021_data_energy.csv", sep = ",")
...
GLM21envir <- glm(df2021$ENVIRONMENTIMPACTCURRENT ~ ., data = df2021, family = gaussian())
summary(GLM21envir)

dat2 <- data[,-c(1:3,35)]
dat2 <- as.data.frame(lapply(dat2, as.numeric))
str(dat2)
GLM21energ<- glm(CURRENTENERGYEFFICIENCY ~ ., dat2,family = gaussian())
summary(GLM21energ)
...
plot_summs(GLM21envir, GLM21energ, robust = list(FALSE, "HC0", "HC5"),
